...
class EventosController extends Controller
{
	...
	{
		$model=new Eventos;
        $path_picture = realpath( Yii::app( )->getBasePath( )."/../uploads/imagenes" )."/";//ruta final de la imagen
 		$path_pdf = realpath( Yii::app( )->getBasePath( )."/../uploads/documentos" )."/";//ruta final de la imagen
		...
		if(isset($_POST['Eventos']))
		{
            ...
            $uploadedFile=CUploadedFile::getInstance($model,'logotipo');
            $uploadedPDF=CUploadedFile::getInstance($model,'convocatoria');
            $fileName = "{$uploadedFile}";  // random number + file name or puedes usar: $fileName=$uploadedFile->getName();
            $pdfName = "{$uploadedPDF}";
            $model->logotipo=$fileName;
            $model->convocatoria=$pdfName;
            ...
            if(!empty($uploadedPDF))  // check if uploaded file is set or not
            {
                //$uploadedFile->saveAs(Yii::app()->basePath.'/../banner/'.$fileName);  // image will uplode to rootDirectory/banner/
                $uploadedPDF->saveAs($path_pdf.$pdfName);
